...
// Extract video ID from URL with improved regex patterns
  ...
  try {
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        // Validate video ID format (always 11 characters)
...
export async function fetchSingleVideo(videoId: string, apiKey: string): Promise<VideoDetails> {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        throw new Error("Video not found. Please check the video URL.")
      } else {
    ...
    if (!data.items || data.items.length === 0) {
      throw new Error("Video not found or is private. Please ensure the video exists and is set to public.")
    }
    ...
    console.error("Error in fetchSingleVideo:", error)
    throw error
  }
}
