...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try to extract playlist ID first (handles cases with both list and v params)
    const playlistId = extractPlaylistId(url)
    
    // Try to extract video ID
    const videoId = extractVideoId(url)

    // Prefer playlist if both are found (e.g., url has both v and list params)
    const isPlaylist = !!playlistId
    const isSingleVideo = !!videoId && !isPlaylist

    if (!isPlaylist && !isSingleVideo) {
      return NextResponse.json(
        {
          error: "Invalid YouTube URL. Please ensure you're using a valid YouTube playlist or video link.",
        },
        { status: 400 },
      )
    }
    ...
    try {
      if (isPlaylist) {
        ...
        if (!/^[a-zA-Z0-9_-]{11}$/.test(videoId)) {
          return NextResponse.json(
            {
              error: "Invalid video ID format. Please check your video URL.",
            },
            { status: 400 },
          )
        }
        ...
        console.log("Video details fetched successfully:", video.title)

        // Calculate stats for single video
        const videos = [video]
        const stats = calculatePlaylistStats(videos)
